...
export const getAllServiceLists = async (req, res) => {
  try {
    ...
    if (Object.keys(req.query).length !== 0) {
      serviceLists = await ServiceList.findOne(req.query);
    } else {
      serviceLists = await ServiceList.find().populate("doctorId patientId");
    }
...
export const getServiceListById = async (req, res) => {
  try {
    const serviceList = await ServiceList.findById(req.params.id).populate(
      "doctorId patientId"
    );
    if (!serviceList) {
